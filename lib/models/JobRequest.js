...
// The worker job feed filters on status + city. (The city match is a
// case-insensitive regex, which can't seek a btree — Mongo narrows on the
// `status` prefix and scans the range. Still the right shape, and the prefix
// alone is worth having.)
jobRequestSchema.index({ status: 1, "location.city": 1 });
