...
export async function adminLogin(username, password) {
  const result = await apiPost("/api/admin/auth/login", { username, password });
  if (result.token && typeof window !== "undefined") {
    localStorage.setItem("kaamsetu_admin_token", result.token);
    localStorage.setItem("kaamsetu_admin", JSON.stringify(result.admin));
  }
  return result;
}
