...
export async function getAllCategoriesForSearch({ force = false } = {}) {
  ...
  try {
    ...
    // A custom row whose slug matches a built-in is a leftover (e.g. a category
    // that later got promoted to built-in, like Labour) — the built-in wins,
    // and listing both would duplicate it in every dropdown.
    const builtinSlugs = new Set(CATEGORIES.map(c => c.slug));
    const customEntries = customs
      .filter(c => c?.slug && c?.nameEn && !builtinSlugs.has(c.slug)) // defensive — bad rows skipped
      .map(c => ({
        id: c.slug,
        slug: c.slug,
