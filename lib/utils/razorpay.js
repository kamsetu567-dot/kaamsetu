...
export function verifySignature(orderId, paymentId, signature) {
  ...
  return hmacEquals(secret, `${orderId}|${paymentId}`, signature);
}

// Verify a Razorpay WEBHOOK signature. Razorpay signs the RAW request body
// with HMAC-SHA256 using the webhook secret (separate from the key secret) and
// sends it in the `x-razorpay-signature` header. The caller MUST pass the raw
// body string exactly as received — parsing to JSON first changes the bytes
// and breaks verification. Returns false on missing config/input or mismatch.
export function verifyWebhookSignature(rawBody, signature) {
  const secret = process.env.RAZORPAY_WEBHOOK_SECRET;
  if (!secret || !rawBody || !signature) return false;
  return hmacEquals(secret, rawBody, signature);
}

function hmacEquals(secret, payload, signature) {
