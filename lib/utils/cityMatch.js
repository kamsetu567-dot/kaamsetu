...
// The map provider returns a real city for urban addresses but falls back to the
// DISTRICT name for rural/semi-urban ones (see mapplsReverse: `r.city ||
// r.district`). That stores one place under two names — "Muzaffarnagar" and
// "Muzaffarnagar District" — which, under the deliberately-strict matching
// below, never match. A job in "Muzaffarnagar District" was invisible to every
// Muzaffarnagar worker. Indian districts are named after their principal city,
// so dropping the suffix is the correct join.
export function stripDistrictSuffix(city) {
  return String(city ?? "").trim().replace(/\s+district$/i, "").trim();
}
