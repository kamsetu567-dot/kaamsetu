...
export async function PATCH(request, { params }) {
  try {
    ...
    if (action === "unblock") {
      // Unblocking restores access but does NOT grant subscription time — the
      // worker keeps whatever subscription window they had. If it's expired,
      // they must pay to receive jobs again (or admin can "extend" as a comp).
    ...
    // Approval verifies identity only — it does NOT grant a subscription.
    // The worker must pay (Razorpay) before their subscription becomes active
    // and the job feed unlocks. Admins can still grant time manually via the
